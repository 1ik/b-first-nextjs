...
export function FeatureTrashTagList() {
  ...
  const { data } = useGet(`api/v1/trash-items/tag`);
  const { request: deleteRequest } = useDelete(`api/v1/delete-trash-item/tag/${deleteId}`);
  const { request: restoreRequest } = usePut(`api/v1/restore-trash-item/tag/${restoreId}`);
