...
export function SquareGrid({ data, className, size = "md", showAccentHeader = false, gridCols }: SquareGridProps) {
  return (
    <div className={`grid grid-cols-${gridCols} gap-8 ${className}`}>
      {data?.map((item: any, index: any) => (
        <div key={index}>
          ...
          <ItemCardVertical data={item} size={size} titleFontSize="18px" />
        </div>
      ))}
    </div>
  );
