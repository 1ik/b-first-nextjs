...
export function FeatureTrashAuthorList() {
  ...
  const { data } = useGet(`api/v1/trash-items/author`);
  const { request: deleteRequest } = useDelete(`api/v1/delete-trash-item/author/${deleteId}`);
  const { request: restoreRequest } = usePut(`api/v1/restore-trash-item/author/${restoreId}`);
