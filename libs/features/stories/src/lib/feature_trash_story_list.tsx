...
export function FeatureTrashStoryList() {
  ...
  const { data } = useGet(`api/v1/trash-items/story`);
  const { request: deleteRequest } = useDelete(`api/v1/delete-trash-item/story/${deleteId}`);
  const { request: restoreRequest } = usePut(`api/v1/restore-trash-item/story/${restoreId}`);
